        ...
        if(root != "fail"):
            ...
            for child in root:  
                ...
                if(any(word in transcript_section for word in ee_uh)):
                    ...
                    if(duration < end):
                        print("clip end duration is longer than the video")
                        end = end-2
                        print("this clip may not contain the target word as there is an edge case issue with the time frame of this clip")
                        start = start - 4
                    if(start == end):
                        start = start -1
...
def main():
    ...
    link_list, id_list = getLinks()
    ee_uh_words, ee_uh = getTargetWords()
    scrapeVideos(link_list, id_list, ee_uh_words, ee_uh)
